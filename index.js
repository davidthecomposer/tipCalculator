...
const inputConstrain = (event) => {
	...
	let decimalFilter = event.target.value
		.split("")
		.filter((char) => char === ".");
	let dollarFilter = event.target.value
		.split("")
		.filter((char) => char === "$");

	//Constraints for Meal Price, Tip Amount and Total Cases:
	if (
		event.target.name === "mp" ||
		event.target.name === "ta" ||
		event.target.name === "total"
	) {
		if (
			decimalFilter.length === 1 &&
...
submit.onclick = () => {
	...
	let percentageTip =
		parseFloat(pt.value.substring(0, pt.value.length - 1)) / 100;
